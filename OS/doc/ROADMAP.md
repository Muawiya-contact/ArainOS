# ArainOS Development Roadmap: 8 Phases to a Fully Functional OS

## Executive Summary

This roadmap outlines the construction of ArainOS—a teaching-oriented 64-bit operating system for x86_64 architecture. The project progresses through 8 phases from basic bootloader to a usable userland environment with a GUI.
...
- QEMU boots, prints a "Hello ArainOS" banner

**Verification:**

```bash
make iso && make run
# Output: "Hello ArainOS"
```
...
- `arch/x86_64/cpu/gdt.c`: GDT descriptor setup
- `arch/x86_64/mm/paging.c`: Page table initialization
- Updated `arch/x86_64/boot/entry.asm`: long mode transition code
- Higher-half kernel mapping (0xFFFFFFFF80000000+)

**Testing:**

- Kernel prints "ArainOS x86_64" before long-mode transition
